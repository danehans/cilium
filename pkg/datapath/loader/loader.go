...
func NewLoader(cfg Config, sysctl sysctl.Sysctl, db *statedb.DB, nodeAddrs statedb.Table[tables.NodeAddress]) datapath.Loader {
	return newLoader(cfg, sysctl, db, nodeAddrs)
}

func newLoader(cfg Config, sysctl sysctl.Sysctl, db *statedb.DB, nodeAddrs statedb.Table[tables.NodeAddress]) *loader {
	return &loader{
		cfg:               cfg,

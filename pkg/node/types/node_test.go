...
func (s *NodeSuite) TestGetNodeIP(c *C) {
	n := Node{
		Name: "node-1",
		IPAddresses: []Address{
			{IP: "192.0.2.3", Type: addressing.NodeExternalIP},
		},
	}
	ip := n.GetNodeIP(false)
	// Return the only IP present
	c.Assert(pkgip.Equal(ip, "192.0.2.3"), Equals, true)

	n.IPAddresses = append(n.IPAddresses, Address{IP: "192.0.2.3", Type: addressing.NodeExternalIP})
	ip = n.GetNodeIP(false)
	// The next priority should be NodeExternalIP
	c.Assert(pkgip.Equal(ip, "192.0.2.3"), Equals, true)

	n.IPAddresses = append(n.IPAddresses, Address{IP: "198.51.100.2", Type: addressing.NodeInternalIP})
	ip = n.GetNodeIP(false)
	// The next priority should be NodeInternalIP
	c.Assert(pkgip.Equal(ip, "198.51.100.2"), Equals, true)

	n.IPAddresses = append(n.IPAddresses, Address{IP: "2001:DB8::1", Type: addressing.NodeExternalIP})
	ip = n.GetNodeIP(true)
	// The next priority should be NodeExternalIP and IPv6
	c.Assert(pkgip.Equal(ip, "2001:DB8::1"), Equals, true)

	n.IPAddresses = append(n.IPAddresses, Address{IP: "2001:DB8::2", Type: addressing.NodeInternalIP})
	ip = n.GetNodeIP(true)
	// The next priority should be NodeInternalIP and IPv6
	c.Assert(pkgip.Equal(ip, "2001:DB8::2"), Equals, true)
...
func (s *NodeSuite) TestGetIPByType(c *C) {
	...
	n = Node{
		Name: "node-2",
		IPAddresses: []Address{
			{IP: "f00b::1", Type: addressing.NodeCiliumInternalIP},
		},
	}
	...
	c.Assert(pkgip.Equal(ip, "f00b::1"), Equals, true)

	n = Node{
		Name: "node-3",
		IPAddresses: []Address{
			{IP: "192.42.0.3", Type: addressing.NodeExternalIP},
			{IP: "f00d::1", Type: addressing.NodeExternalIP},
		},
	}
	...
	ip = n.GetIPByType(addressing.NodeExternalIP, false)
	c.Assert(pkgip.Equal(ip, "192.42.0.3"), Equals, true)
	ip = n.GetIPByType(addressing.NodeExternalIP, true)
	c.Assert(pkgip.Equal(ip, "f00d::1"), Equals, true)
}
...
func (s *NodeSuite) TestParseCiliumNode(c *C) {
	...
	c.Assert(n, checker.DeepEquals, Node{
		...
		IPv4HealthIP:            "1.1.1.1",
		IPv6HealthIP:            "c0de::1",
		IPv4IngressIP:           "1.1.1.2",
		IPv6IngressIP:           "c0de::2",
